...
<template>
  <base-card-wrapper v-if="subjects && subjects.length > 0">
    <template #items>
      <base-card v-for="subject in subjects" :key="subject.id">
        <template #title>
          {{subject.name}}
        </template>
        <template #body>
        ...
        <template #actions>
          <button class="base-card__button" @click="store.dispatch(actionsTypes.SET_ACTIVE_SUBJECT, subject)">Перейти</button>
        </template>
      </base-card>
    </template>
  </base-card-wrapper>
  <empty-message v-else>
    ...
    У тебя нет доступных предметов
  </empty-message>
</template>
